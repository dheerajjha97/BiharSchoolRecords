        ...
        <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2 md:col-span-2 relative">
              ...
              <Input id="udise" placeholder="11-digit UDISE code" value={udise} onChange={(e) => setUdise(e.target.value)} disabled={!!firebaseError} />
              {isLookingUp && <Loader2 className="absolute right-3 top-9 h-4 w-4 animate-spin" />}
            </div>
            <div className="space-y-2 md:col-span-2">
              <Label htmlFor="school-name">School Name</Label>
              <Input id="school-name" placeholder="Official school name" value={schoolName} onChange={(e) => setSchoolName(e.target.value)} disabled={!!firebaseError} />
            </div>
            <div className="space-y-2 md:col-span-2">
              <Label htmlFor="school-address">School Address</Label>
              <Input id="school-address" placeholder="Full school address" value={address} onChange={(e) => setAddress(e.target.value)} disabled={!!firebaseError} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="email">Email Address</Label>
              <Input id="email" type="email" placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} disabled={!!firebaseError} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="mobile">Mobile Number</Label>
              <Input id="mobile" type="tel" placeholder="+919876543210" value={mobile} onChange={(e) => setMobile(e.target.value)} disabled={!!firebaseError} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="password">Password</Label>
              <Input id="password" type="password" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} disabled={!!firebaseError} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="confirm-password">Confirm Password</Label>
              <Input id="confirm-password" type="password" placeholder="••••••••" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={!!firebaseError} />
            </div>
          </div>
          {error && (
          ...
          <Button onClick={handleSignup} disabled={isLoading || isLookingUp || !!firebaseError} className="w-full mt-6">
